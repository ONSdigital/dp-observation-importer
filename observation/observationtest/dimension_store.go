...
import neoErrors "github.com/johnnadratowski/golang-neo4j-bolt-driver/errors"
...
func (ds *DimensionStore) GetOrder(instanceID string) ([]string, error) {
	if ds.ReturnError {
		return []string{}, neoErrors.New("Failed to fetch csv header")
	}
...
func (ds *DimensionStore) GetNodeIDs(instanceID string) (map[string]string, error) {
	...
	if ds.ReturnError {
		return nodeMap, neoErrors.New("Failed to fetch node ids")
	}
